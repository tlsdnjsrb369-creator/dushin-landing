...
export const metadata = {
  title: "면허 및 자격증 | (주)두신이엔지",
  description: "(주)두신이엔지 보유 면허 및 자격증을 확인하세요.",
};

// ── 자격증/면허 사진이 준비되면 아래 배열에 항목을 추가하세요 ────────────
// 예시:
// { src: "/cert_welding.jpg", name: "용접기술사 자격증", issuer: "한국산업인력공단", year: "2020" }
...
export default function CertificatesPage() {
  const hasItems = CERTIFICATE_ITEMS.length > 0;
        ...
        <div className="mb-14 border-b border-slate-200 pb-10">
          ...
          <div>
            ...
            <p className="text-slate-500 text-sm md:text-base leading-relaxed max-w-2xl">
              (주)두신이엔지가 보유한 각종 면허, 인증서 및 자격증을 소개합니다.
              아래 카드 영역에 이미지를 추가하여 언제든지 업데이트할 수 있습니다.
            </p>
          </div>
        </div>

        {/* 자격증 그리드 — 자격증은 A4·세로 비율(3/4)이 어울림 */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {hasItems
            ? CERTIFICATE_ITEMS.map((item, idx) => (
                <div
                  key={idx}
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    {item.issuer && (
                      <span className="text-xs text-slate-400 font-medium">{item.issuer}</span>
                    )}
                    {item.year && (
                      <span className="ml-2 text-xs font-bold text-brand-blue">{item.year}</span>
                    )}
                  </div>
                </div>
              ))
            : Array.from({ length: PLACEHOLDER_COUNT }).map((_, idx) => (
                <div
                  key={idx}
                  className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/60 aspect-[3/4] p-8 text-center hover:border-brand-blue hover:bg-brand-blue/5 transition-all duration-300 group"
                >
                  {/* 문서 아이콘 */}
                  <svg
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-slate-300 leading-relaxed">
                <code className="text-brand-blue font-mono text-xs bg-slate-800 px-2 py-0.5 rounded">
                  src/app/archives/certificates/page.js
                </code>{" "}
                파일 상단의{" "}
                <code className="text-brand-blue font-mono text-xs bg-slate-800 px-2 py-0.5 rounded">CERTIFICATE_ITEMS</code> 배열에 항목을 추가하고, 이미지를{" "}
